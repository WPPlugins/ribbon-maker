...
function ribbon_maker_admin_menu() {
	...
	echo('
		...
		<td><input type="text" name="ribbon_maker_title'.$slot.'" size="80" value="'.get_option( "ribbon_maker_title$slot",'Get Support at BowieRocks.com for Ribbon Maker' ).'"></td>
		</tr>
		...
		<td><input class="color" type="text" name="ribbon_maker_bgcolor'.$slot.'" size="10" maxlength=7 value="'.get_option( "ribbon_maker_bgcolor$slot",'000000' ).'"></td>
		</tr>
		<tr><td>FGCOLOR of Ribbon<br/><span class="ribbon_maker_hint">This is the color of the text comprising the message.</span></td>
		<td><input class="color" type="text" name="ribbon_maker_fgcolor'.$slot.'" size="10" maxlength=7 value="'.get_option( "ribbon_maker_fgcolor$slot",'ffffff' ).'"></td>
		</tr>
...
function ribbon_maker_admin_head() {
	echo( '
		<script type="text/javascript" src="'.plugins_url("jscolor/jscolor.js",__FILE__).'"></script>
		<style type="text/css">
...
function render_ribbon_maker() {
	...
	if(get_option("ribbon_maker_active$i")=="active") {
		...
		echo "<a target='_blank' class='ribbon-maker' href='". get_option("ribbon_maker_url$i")."' title='".get_option("ribbon_maker_title$i")."'><img src='{$img}' alt='Ribbon Maker' style='position: fixed; $tob: {$offset}px; $lor: {$h_offset}px; z-index: 0; cursor: pointer;' /></a>";
	}
}
